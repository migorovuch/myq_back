  ...
  stages {
    ...
    stage('Run PHP Unit tests') {
      steps {
        ...
        sh 'docker cp myq_app_test:/var/www/project/var/testResults/phpunit.xml ./testResults.xml'
        sh 'docker cp myq_app_test:/var/www/project/var/testResults/clover.xml ./clover.xml'
    ...
    stage('Run environment') {
      steps {
        ...
        sh 'docker run --rm -t -d --network=myq_network -p 3307:3306 --name myq_mysql --env-file .env myq_mysql'
      }
    }
