  ...
  stages {
    ...
    stage('Run TEST environment') {
      steps {
        ...
        sh 'docker run --rm -t -d --network=myq_network -p 3307:3306 --name myq_mysql_test --env-file .env.test myq_mysql'
...
//         sh 'until docker exec myq_mysql bash "mysqladmin ping"; do >&2 "MySQL is unavailable - sleeping"; sleep 2; done'
    ...
    stage('Copy FRONT to Nginx container') {
        steps {
            withCredentials([string(credentialsId: 'volumes_path', variable: 'VOLUMES_PATH')]) {
                sh "docker cp $VOLUMES_PATHfront/. myq_nginx:/var/www/project/public/front/"
            }
        }
    }
